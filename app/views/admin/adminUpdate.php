<?php
$this->layout('admin/adminMaster');
use app\routes\Uri;
use app\database\models\querys\SelectAdminId;
...
foreach (SelectAdminId::exec() as $arr) {
    $name = htmlspecialchars($arr['name']);
    $name2 = htmlspecialchars($arr['name2']);
    $email = htmlspecialchars($arr['email']);
  ;}
  ?>
  <div id="formUpdateAdmin">
    <form method="POST" action="/admin/update/<?php echo UriId::exec()?>">
    <h1 style="text-align: center;">Admin Update</h1>
    <div class="form-outline mb-2 d-flex" style="gap:1rem;">
      <input type="text" id="form3Example1cg" placeholder='<?php echo $name ?>' name="name" class="form-control form-control-lg" />
      <input type="text" id="form3Example1cg" placeholder="<?php echo $name2 ?>"  name="name2" class="form-control form-control-lg" />
    </div>
    
    <div class="form-outline mb-2">
      <input type="email" id="form3Example3cg" placeholder="<?php echo $email ?>"  name="email" class="form-control form-control-lg" />
    </div>
